...
class DaEventoCTeNFePHP extends CommonNFePHP implements DocumentoNFePHP
{
    ...
    */
    public function __construct($docXML='', $sOrientacao='',$sPapel='',$sPathLogo='', $sDestino='I',$sDirPDF='', $fontePDF='', $aEnd='', $mododebug=0)
    {
    ...
    */
    ...
    {
        return $this->montaDAEventoCTe($orientacao,$papel,$logoAlign,$situacao_externa,$CLASSE_PDF);
    }//fim monta
     ...
     * @package NFePHP
     * @name montaDAEventoCTe
     * @version ????
     * @param  string $orientacao (Opcional) Estabelece a orientação da impressão (ex. P-retrato), se nada for fornecido será usado o padrão da CTe
     * @param  string $papel      (Opcional) Estabelece o tamanho do papel (ex. A4)
     * @return string O ID do evento extraido do arquivo XML
     */
    public function montaDAEventoCTe($orientacao='',$papel='A4',$logoAlign='C',$situacao_externa=NFEPHP_SITUACAO_EXTERNA_NONE,$CLASSE_PDF=false)
    {
        if ($orientacao == '') {
            $orientacao = 'P';
        }
    ...
    elseif($this->tpEvento=='110111')
        $texto = "Este documento é uma representação gráfica do evento de CTe e foi impresso apenas para sua informação e não possue validade fiscal.\n O Evento deve ser recebido e mantido em arquivo eletrônico XML e pode ser consultada através dos Portais das SEFAZ.";
        ...
        $texto = "DAEventoCTeNFePHP ver. " . $this->version .  "  Powered by NFePHP (GNU/GPLv3 GNU/LGPLv3) © www.nfephp.org";
        $aFont = array('font'=>$this->fontePadrao,'size'=>6,'style'=>'I');
